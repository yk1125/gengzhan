...
- 页头透明顶**全站默认**（2026-09-15 用户裁定：透明不止给首页，全局导航栏都跟首页一样；此条取代 2026-09-14「只给首页」的口径）：透明/实底由 `route.meta.headerTransparent !== false` 得到，当前所有页面均为透明顶；个别要实底的页面显式把 `meta.headerTransparent` 设为 `false`。透明态字色分两档，由 `route.meta.headerInk` 声明：`light` = 深色首屏上的白字（首页 `/` `/en`、关于、案例详情、资讯详情），`dark` = 浅色首屏上的深字（米色服务页、案例/资讯列表、AI 咨询、法律/404 等）。**M-04（`.on` 底色反转）与 M-05（`.hide` 滚轮收放）与皮肤无关，全站生效**——七类服务模板因此白拿同一套行为。页头样式只允许在 `frontend/src/style.css` 的「页头（Header）」段一处定义（靠选择器作用域 + 源码顺序取胜，**新增规则不得再叠 `!important`**）；`frontend/src/layout/components/Header.vue` 只保留汉堡按钮、移动菜单皮肤与 M-03 入场。C/D 的页面不得再写页头样式。
