...
| V11（已决） | 参考站在 1025–1365px 区间**没有页头入场动画**（`function.js:4558` 唯一门槛 `document.body.clientWidth > 1365`，M-03） | **不照抄**（2026-09-14 用户决策）：门槛改为 `clientWidth > 1024`，与统一移动端断点对齐；≤1024 不播放，≥1025 播放。其余参数照抄 M-03 | 该 1365 全库仅此一处、不属参考站断点体系（真实断点为 1024 / 1920）；参考站布局 CSS 在 1025–1365 仍按桌面处理，判定为遗漏而非设计。照抄会得到「时有时无」的页头。页头归 A 的公共层，B/C 不得各自实现 |
