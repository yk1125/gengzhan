...
2026-09-17 路由子项进度：18 项 routeManifest 已补齐，localizeRoute 使用命名 location，详情/404 参数由 Vue Router 解析；首页调用已适配，门禁覆盖双语链接及参数边界。build 与普通路由检查通过；严格路由检查仅剩 Contact/ContactEn 待实现（B/T05）。本次完成不代表 T01 整体冻结；具体证据见 handoffs/A.md 与 INTEGRATION.md。
...
| T00 | 完成 | 方案确认 | A | `9e83917`（T00R 动效 SPEC）+ `9231087`（T00A 客户 Logo 与过渡视频）已并入 main；剩余视觉缺口见 REFERENCE.md |
| T01 | 完成 | T00 | A | foundationCommit `4c18b6a`（main 侧 `512565e`，收敛后由 merge 取代） |
| T02-H | 完成 | T01 | B | 源分支 `9224b4f` → main 合并 `84b7929` |
| T02-S | 完成 | T01 | C | 源分支 `822af9a` → main 合并 `1a9aa2a` |
| 样板验收 | 已通过 | T02-H/S集成 | 用户/A | 用户在本对话确认“目前我验收没什么问题了”；2026-09-17 登记，统一代码基准 pilotCommit `9586ba2` |
| T03 | 可启动，未派发 | 样板已批准 | C | 从统一基准核对服务页剩余内容与双语 |
| T04 | 可启动，未派发 | 样板已批准 | D | 案例/资讯列表与详情 |
| T05 | 可启动，未派发 | 样板已批准 | B | 公司/联系/法律与双语；联系路由仍 PENDING |
| T06 | 可启动，未开始 | 样板已批准 | A | AI/公共收尾与共享数据能力核查 |
| T07 | 未开始 | T03—T06 | A | — |
| T08 | 未开始 | T07 | A | — |

本表于 2026-09-17 按用户验收与路由修复更新。`9586ba2` 包含已验收外壳、暗色滚动导航修复及路由契约补齐；后续交接文档提交不改变代码基准。样板验收门已解除，本轮仅登记与提交，尚未派发 B/C/D。T01 历史“完成”不代表测试与共享数据能力已逐项复核，遗留核查由 A 在 T06 跟进。
