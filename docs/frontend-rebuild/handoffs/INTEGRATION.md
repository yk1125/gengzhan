...
> **最新状态（2026-09-17 登记）**：用户已在本对话确认“目前我验收没什么问题了”，样板验收通过。统一代码基准 `pilotCommit = 9586ba2`，含路由修复。T03/T04/T05 可启动，尚未派发；A 继续 T06 及最终集成。
> 下方各日期的 Git 收敛、验收矩阵与“未提交”等记录为历史快照，保留当时证据。当前状态以本节和 PLAN 跟踪表为准，不能用历史待确认状态重新阻挡下游。
...
| pilotCommit | `9586ba2` | 样板已验收；路由修复后的统一代码基准 |

本次代码提交与交接文档分开：`9586ba2` 为稳定代码基准，后续文档提交登记该实际 hash。验证沿用本轮已执行结果：build PASS，routeManifest 18 项通过，check:routes PASS 34 / FAIL 0 / PENDING 2，修改代码 ESLint 0 errors / 127 首页模板 warnings。严格模式因中英文联系页未实现仍退出 1；共享数据与基础测试能力由 A 在 T06 核查。此次没有向其他会话发送启动通知。
...
### 9. pilotCommit（2026-09-17 更新）

- **pilotCommit = `9586ba2`**，包含公共层收编、主题/语言/页头修复与完整路由对照表。
- 用户已明确验收通过，2026-09-17 登记；旧候选 `1a9aa2a` 仅保留为此前截图矩阵的证据基准，不能作为最新下游起点。
- 路由修复构建与检查结果见文末 T01 记录；此前截图没有针对新基准重新拍摄，不将历史矩阵冒充新基准全量验收。

### 10. 下一步（验收门已解除，通知尚未发送）

样板验收门已通过。下游统一从 `9586ba2` 开始，结合现有成果核对剩余任务；本轮仅更新基准，不派发任务。
...
```text
[给 B] 样板已由用户验收通过，pilotCommit = 9586ba2。从该提交启动 T05：About / Contact / Privacy / Legal + 中英文；表单字段、国际号码 / 邮箱、必填选填、隐私勾选、received / demo / unavailable 按 DATA.md；后台未接通走已批准退路，不伪造电话 / 二维码 / 提交成功。只改 B 所属文件。
[给 D] 样板已由用户验收通过，pilotCommit = 9586ba2。从该提交启动 T04：Cases / News 列表、基础分类 / 分页、详情、返回与咨询；只消费既定 repository，修未知 ID 与数据不一致；不做搜索 / 相关推荐 / 浏览量。
```

### 11. 下次第一步

1. 样板批准与统一代码基准已登记，无需再次确认样板。
2. 后续派发时向 B/D/C 提供 `9586ba2` 与当前规格，核对已完成内容后执行剩余任务。
3. A 继续 T06；IoT 入口、共享数据能力与基础测试核查仍跟进，联系页待 B/T05。
...
### 2026-09-17 · T01 路由契约补齐

起点 main `0acd1f8`，代码与规格已提交为 `9586ba2`，同时作为统一代码基准。18 个 routeKey 全部登记；localizeRoute 返回命名 location，沿用已注册的路由名称，详情参数和 catch-all 编码由 Vue Router 处理。唯一既有调用者 Home 已改为 router.resolve 获取字符串路径。FRONTEND §3 同步说明兼容方式，Contact/ContactEn 为待注册名称。

check:routes PASS 34 / FAIL 0 / PENDING 2，缺少 15 项警告消除；check:routes:strict 的退出码仍为 1，仅因中英文联系页待实现。扩展现有门禁验证名称/路径/SPEC 一致、双语参数编码往返、ID=0/缺失 ID、404 多段路径、query/hash 和非法输入。build PASS；修改代码只读 eslint 0 errors / 127 warnings（首页模板）；git diff --check PASS。无视觉变更、无新增截图、无新增后端接口。下一步由 B/T05 完成联系页后由 A 注册路由并更新 PENDING。
