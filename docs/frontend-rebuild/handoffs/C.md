...
## 32. 用户裁决（2026-09-15，对第 30 节"待裁决"的答复）

| # | 问题 | 用户答复 | 对本轮代码的影响 |
|---|---|---|---|
| ① | 服务名用「数字创意」还是旧官网「数字文创」 | **数字创意** | 无需改动（现状即"数字创意"）；旧官网的「数字文创」不再作为改名依据 |
| ② | 旧官网骨架页里的「数字藏品 / NFT」是否作为真实服务保留 | **先不管** | 不作为卡片加入；保留在 `copy-review-round5.md` 待议 |
| ③ | IoT 页 01 区看板里的示意数字（`98.6%` / `03` / `12` / `稳定`） | **先不动** | 保持现状，不加"界面示意"字样 |
| ④ | 服务页是否展示公司数字（10年+ / 200+ 客户 / 98% / CMMI3 / 24-7） | **不用** | 服务页不加；公司数字仍只在首页（`REQUIREMENTS.md Q18` 口径） |
| ⑤ | en 服务页文案（卡片/标签/流程目前是中文）是否由 C 翻一轮 | **不用**（该事项归 A） | C 不再做 en 翻译；en 服务页维持"英文标题 + 中文卡片/标签/流程"的现状，等 A 的双语/译文管线统一处理 |
